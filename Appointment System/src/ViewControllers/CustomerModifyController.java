...
public class CustomerModifyController implements Initializable {
    ...
    public CustomerModifyController(Customer customer){
    ...
    /**
     * Save Customer Edits with Error Checking
     * @param event
     * @throws IOException
     */
    @FXML
    void SaveClick(MouseEvent event) throws IOException {
        ...
        if(PhoneNumber.getText().trim().isEmpty()){
            ...
            CustomersImplementation.updateCustomer(Integer.parseInt(CustomerID.getText().trim()), Name.getText().trim(), Address.getText().trim(), StateProvince.getSelectionModel().getSelectedItem().toString(), PostalCode.getText().trim(), PhoneNumber.getText().trim());
            ReturnClick(event);
        }
    }

    /**
     * Correctly Populates current Customer
     */
    private void populateCustomer(){
        this.CustomerID.setText((Integer.toString(customer.getCustomerId())));
        this.Name.setText(customer.getCustomerName());
        this.Address.setText(customer.getAddress());
        this.Country.getSelectionModel().select(customer.getCountry());
        this.PostalCode.setText(customer.getPostalCode());
        this.PhoneNumber.setText(customer.getPhone());
    }
    ...
    /**
     * Correctly picks the City for Dropdown
     */
    public void pickCity(){
        for (First_Level_Division first : StateProvince.getItems()) {
            if (first.toString().equals(customer.getDivision())){
                StateProvince.getSelectionModel().select(first);
                break;
            }
        }
    }
